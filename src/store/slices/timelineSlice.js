...
export const getAllTimeline = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      ...
      "https://akkasdevbackendcode-production.up.railway.app/api/v1/timeline/getall",
    ...
    dispatch(
      timelineSlice.actions.getAllTimelineFailed(error)
    );
  }
};
...
export const addNewTimeline = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      "https://akkasdevbackendcode-production.up.railway.app/api/v1/timeline/add",
      data,
      {
        withCredentials: true,
    ...
    dispatch(
      timelineSlice.actions.addNewTimelineFailed(error)
    );
  }
};
export const deleteTimeline = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://akkasdevbackendcode-production.up.railway.app/api/v1/timeline/delete/${id}`,
      {
        withCredentials: true,
      }
    );
    ...
    dispatch(
      timelineSlice.actions.deleteTimelineFailed(error)
    );
  }
};
