...
export const getAllMessages = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      "https://akkasdevbackendcode-production.up.railway.app/api/v1/message/getall",
    ...
    dispatch(
      messageSlice.actions.getAllMessagesFailed(error)
    );
  }
};
...
export const deleteMessage = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://akkasdevbackendcode-production.up.railway.app/api/v1/message/delete/${id}`,
      {
        withCredentials: true,
      }
    );
    ...
    dispatch(
      messageSlice.actions.deleteMessageFailed(error)
    );
  }
};
